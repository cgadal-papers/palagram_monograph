...
for i, d in enumerate(datasets):
    if 'rho_f' in d.variables.keys():
        t = d.variables['t'][:].data
        x_front = d.variables['x_front'][:].data
        #
        if np.isnan(d.variables['H0'][:].data):
            H = d.variables['Ha'][:].data
        else:
            H = d.variables['H0'][:].data
        #
        if np.isnan(d.variables['alpha'][:].data):
            alpha = 0
        else:
            alpha = d.variables['alpha'][:].data
        #
        if np.isnan(d.variables['rho_f'][:].data):
            rho_f = 1000
        else:
            rho_f = d.variables['rho_f'][:].data
        #
        if np.isnan(d.variables['rho_a'][:].data):
            rho_a = 1000
        else:
