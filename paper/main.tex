...
For a finite volume lock-release device on a mild enough slope, the slumping regime necessarily ends when the rarefaction wave associated with finite volume reaches the current front \citep[e.g.][]{Rottman1983, Hogg2006}. This leads to a new regime usually referred to as the self-similar regime in the literature. This is observed on the front evolution as $U_{\text{c}}$ is no longer time independent but evolves as $t^{-1/3}$.
