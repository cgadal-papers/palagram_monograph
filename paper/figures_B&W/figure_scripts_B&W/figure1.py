...
figsize = (tp.large_figure_width, 0.9 * tp.large_figure_width)
fig = plt.figure(figsize=figsize, layout="compressed")

subfigs = fig.subfigures(2, 1, height_ratios=(0.339, 0.6603), hspace=0.005)

# %%%%%%% Sketches

axs_sketches = subfigs[0].subplots(1, 3, width_ratios=width_ratios, gridspec_kw={"wspace": 0.02})

ax_labels = ["a", "b", "c"]
for i, (ax, ax_label, slope, water_height_fact) in enumerate(
    zip(axs_sketches.flatten(), ax_labels, SLOPES, water_height_facts)
):
    ax.set_aspect("equal")
    ax.set_axis_off()
    # ax.set_xticks([])
    ...
    #
    if ax_label in ["a", "b"]:
        ax.text(bottom_right[0] - 0.4 * tank_length, 0.75 * y.mean(), r"$\alpha$", ha="right", va="center")
    ...
    ax.text(xytext[0], xytext[1], r"$L_{1}$", ha="center", va="top")
    #
    xy = bottom_left - 0.195 * door_height * slope_vec_up
    xytext = bottom_door - 0.195 * door_height * slope_vec_up
    ax.annotate("", xytext=xytext, xy=xy, arrowprops=dict(arrowstyle="<->", shrinkA=0, shrinkB=0, color="k"))
    xytext = (xy + xytext) / 2
    ax.text(xytext[0], xytext[1] - 0.06 * door_height, r"$L_{0}$", ha="center", va="top")

# %%%%%%% experimental images
subfigures_images = subfigs[1].subfigures(3, 1, height_ratios=(0.75, 0.10, 0.2), hspace=0.022 / 2)

ax_line = subfigures_images[1].subplots(1, 1)
ax_line.hlines(y=0.5, xmin=0.3, xmax=0.7, transform=ax_line.transAxes, color="k")
ax_line.set_axis_off()

subfigures_images_top = subfigures_images[0].subfigures(1, 3, width_ratios=width_ratios)

# ## IMFT (sand80m_H19/run03)
path_imgs = "src/images_figure1/IMFT"
images = [plt.imread(img) for img in sorted(glob.glob(os.path.join(path_imgs, "*.tif")))]

axs_IMFT = subfigures_images_top[0].subplots(len(images), 1, gridspec_kw={"hspace": 0.03})
...
axs_LEGI = subfigures_images_top[1].subplots(len(images), 1, height_ratios=img_sizes, gridspec_kw={"hspace": 0.02})
...
images = [plt.imread(img) for img in sorted(glob.glob(os.path.join(path_imgs, "*.tiff")))]

axs_LEMTA = subfigures_images_top[2].subplots(len(images), 1, gridspec_kw={"hspace": 0.03})
...
data = np.load(os.path.join(path_imgs, "run07_snpachots_extracts.npy"), allow_pickle=True).item()

X, Y = np.meshgrid(data["x"], data["y"])

axs_SEDFOAM = subfigures_images[2].subplots(
...
for ax, arr_phi in zip(axs_SEDFOAM[:, 1:-1].flatten(), data["phi"]):
    ax.pcolormesh(X.T, Y.T, arr_phi, cmap=tp.cmap_images2, vmax=0.025, rasterized=True)
    ax.set_aspect("equal")
    ax.set_xticks([])
    ax.set_yticks([])
...
axs = np.concatenate(
    [
        axs_sketches.flatten(),
        axs_IMFT.flatten(),
        axs_LEGI.flatten(),
        axs_LEMTA.flatten(),
        axs_SEDFOAM[:, 1:-1].flatten(),
    ]
)

for l, ax in zip(letters[:3], axs[:3]):
    trans_base = mtransforms.blended_transform_factory(ax.transAxes, subfigs[0].transFigure)
    trans = mtransforms.ScaledTranslation(1 / 72, -5 / 72, fig.dpi_scale_trans)
    label = f"({l})"
    subfigs[0].text(0, 1, label, color="k", transform=trans_base + trans, va="top", ha="left")
...
for l, ax in zip(letters[3:], axs[3:]):
    ...
    ax.text(
        ...
        va="top",
        ha="right",
        bbox=dict(facecolor="w", edgecolor="none", alpha=0.8, pad=2.2),
    )


# plt.show()
fig.savefig("../{}.pdf".format(sys.argv[0].split(os.sep)[-1].replace(".py", "")), dpi=400)
